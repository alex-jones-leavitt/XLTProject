...
<html lang="en">
  ...
  <body class="text-center">
    <form class="form-signin" method="post">
      ...
      <h1 class="h3 mb-3 font-weight-normal">Please Enter Your Login Info</h1>
      ...
      <input type="password" id="inputPassword" name="newpassword1" class="form-control" placeholder="New Password" required>
	  <label for="inputPassword" class="sr-only">Repeat New Password</label>
      <input type="password" id="inputPassword" name="newpassword2" class="form-control" placeholder="New Password" required>
      <button class="btn btn-lg btn-primary btn-block" type="submit" style="background-color:#8b2233">Reset Password</button>
	  <br>
...
if(isset($_POST["email"])){
	...
	if($loginresult = mysqli_fetch_assoc($loginquery)) {
		$loginSuccess = password_verify($password, $loginresult['Password']);
		if($loginSuccess) {
			if(!strcasecmp($_POST['newpassword1'], $_POST['newpassword2'])){
				$newpassword1 = $_POST['newpassword1'];
				//Compare login data
				#echo "<script> alert('".password_hash($password, PASSWORD_DEFAULT)." \\n ".$loginresult['Password']."');</script>";
				$newpasswordinsert = password_hash($newpassword1, PASSWORD_DEFAULT);
				$updatepsql = "UPDATE User SET Password = '".$newpasswordinsert."' WHERE Email like '".$email."';";
